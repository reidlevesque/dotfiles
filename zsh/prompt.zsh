...
function setKubernetesPrompt {
  if [[ -f "${HOME}/.kube/config" ]]; then
    ...
    local curnamespace=$(cat ~/.kube/config | yq ".contexts.[].context | select(.cluster | contains(\"$curcontext\")) | .namespace" | head -n 1)
    ...
    elif [[ "${curkube}" =~ ".*-d-.*" || "${curkube}" =~ ".*-x-.*" || "${curkube}" =~ ".*-pre.*" ]]; then
      # DEV
      k8s_prompt_colour=$FG[002]
    elif [[ "${curkube}" =~ ".*-s-.*" || "${curkube}" =~ ".*-infra.*" || "${curkube}" =~ "lab.*ts\.net"  ]]; then
      # STAGING
      k8s_prompt_colour=$FG[003]
    elif [[ "${curkube}" =~ ".*-p-.*" || "${curkube}" =~ ".*-prod.*" || "${curkube}" =~ "prod.*ts\.net" ]]; then
      # PROD
